## Перечень задач в виде плана. :point_down:
   ...
   - [x] 17. Ознакомиться с документацией по **axios**: **https://github.com/axios/axios**
        ...
        - [x] 17. 1. Разобраться с необходимыми заголовками для выполнения кросс-доменных запрсов (*Проблема, axios из коробки не поддерживает крос-доменные запросы :/*);
   ...
   - [x] 18. Составить кросдоменный get-запрос на получение 10 юзеров, запросить фото, имя и фамилию на VK API.
        ...
        - [x] 18. 3. Реализовать логику выполнения запроса в методе: getUsersJSONP().

        - [x] 18. 4. В компоненте: userDetails.vue реализовать метод: getUser() для получения пользователя через users.get по id юзера.

   - [x] 19. Настроить передачу параметров между компонентами приложения:

        - [x] 19. 1. Из компонента: **navBar.vue** из поля поиска передать значение параметра query на компонент userList.

        - [x] 19. 2. Из компонента: **userList.vue** по запросу осуществить вывод тестового объекта(ов).

        - [x] 19. 4. Из компонента: **userListItem** по клику на кнопку: **Подробнее** передать id выбранного пользователя на компонент: **userDetails.vue**.

        - [x] 19. 5. В компоненте **userDetails.vue** отобразить необходимые данные о пользователе.

   - [x] 20. Реализовать в проекте бесконечную прокрутку на главной странице:

        - [x] 20. 1. При запросе пользователей в методе: getUsersJSONP(), в компоненте userList.vue отобразить прелоадер: **Идёт загрузка пользователей...**.

        - [x] 20. 2. В компоненте: userList.vue при релизации запроса на получение юзеров реализовать смещение результатов на 10 юзеров при помощи параметра, предоставляемого VK API: *offset*.

        - [x] 20. 3. При скролле страницы вниз реализовать автоподгрузку контента:

            - [x] 20. 3. 1. Ознакомиться с работой по работе с размером и прокруткой страницы: *document.body.scrollHeight* (высота страницы с учётом прокрутки), *window.innerHeight* (текущий размер окна браузера), *document.scrollingElement.scrollTop* (текущая прокрутка в документе).

            - [x] 20. 3. 2. В хуке: **created** добавить слушателя на событие: *scroll*;

            - [x] 20. 3. 3. Добавить метод: **onScroll**, в котором реализовать логику обработки прокрутки и выполнение метода: **getUsersJSONP()** при необходимых условиях (*т.е. страница прокручена до низу*)

         - [x] 20. 4. Добавить вывод надписи: "Пользователей не найдено!" в компонент: userList.vue.

   - [x] 21. Реализовать появление кнопки: "НАЗАД", если пользователь першёл в компонент: vueDetails.vue и сокрытие данной кнопки на всех остальных компонентах.
