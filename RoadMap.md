## Перечень задач в виде плана. :point_down:
   ...
   - [ ] 17. Настроить передачу параметров между компонентами приложения:

        - [x] 17. 1. Из компонента: **navBar.vue** из поля поиска передать значение параметра query на компонент userList.

        - [x] 17. 2. Из компонента: **userList.vue** по запросу осуществить вывод тестового объекта(ов).

        - [ ] 17. 4. Из компонента: **userListItem** по клику на кнопку: **Подробнее** передать id выбранного пользователя на компонент: **userDetails.vue**.

        - [ ] 17. 5. В компоненте **userDetails.vue** отобразить необходимые данные о пользователе.
